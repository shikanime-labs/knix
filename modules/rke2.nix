...
in
{
  config = mkIf cfg.enable {
    ...
    networking = {
      # Disable IPv6 privacy extensions (use_tempaddr). Canal host-gw uses
      # node global IPv6 addresses as route next-hops for cross-node pod
      # traffic. Rotating temporary addresses expire and leave next-hops
      # permanently FAILED, breaking IPv6 cross-node pod routing and the
      # IPv6 kube-dns VIP. The native option also sets udev rules so new
      # interfaces inherit it, which raw sysctl cannot do.
